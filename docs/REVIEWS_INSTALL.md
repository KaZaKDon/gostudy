...
> **Архивная инструкция PHP/MySQL.** Фиксирует готовое поведение отзывов для
> переноса в модуль `reviews` NestJS. Указанные PHP-файлы не являются
> производственным backend.

Актуальная реализация PostgreSQL/NestJS описана в
[`REVIEWS_API_NODE.md`](REVIEWS_API_NODE.md). Инструкции ниже сохранены только
как история и больше не применяются к текущему проекту.
...
Миграция проверяет существование каждого столбца, индекса и внешнего ключа,
поэтому её безопасно запускать и на ранее подготовленной таблице `reviews`.
Если предыдущая версия миграции остановилась с ошибкой `#1060`, нужно просто
запустить этот исправленный файл целиком.
