...
Связи между областями объявляются через `imports`/`exports` NestJS-модулей.
Общие технические правила находятся в `common`; общие бизнес-контракты должны
экспортироваться через публичные сервисы либо выделенные файлы контрактов.
Полный DI-граф проверяется командой `npm run test:modules` в `server`.
...
Frontend не обращается к legacy PHP API. Новые области подключаются только
после появления проверенного маршрута NestJS.
...
- несовершеннолетнего ученика регистрирует родитель; самостоятельная
  регистрация ученика требует точную дату рождения и возраст не менее 18 лет;
...
- файлы преподавателя обслуживаются отдельными модулями документов и медиа с
  приватным хранилищем и административной модерацией.
