...
| `PARENT_STUDENT_LINK_API_NODE.md` | Привязка подтверждённой карточки к существующему аккаунту ученика |
| `PARENT_STUDENT_ACCOUNT_API_NODE.md` | Создание и немедленная привязка нового аккаунта ученика родителем |
| `PARENT_CHILD_NOTIFICATIONS.md` | Внутренние уведомления родителя о событиях привязанных детей |
| `PARENT_NOTIFICATION_SETTINGS.md` | Настройки категорий уведомлений отдельно для каждого ребёнка |
| `PARENT_DASHBOARD_API_NODE.md` | Семейная главная страница родителя со сводкой по всем детям |
