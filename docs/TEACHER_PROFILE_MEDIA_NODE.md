...
- `GET /api/v1/teachers/profile-media/:id` — только подтверждённый файл активного преподавателя с подтверждённой анкетой; для видео поддерживаются Range-запросы.

Блокировка, архивация или удаление аккаунта закрывают публичную выдачу ранее
подтверждённых файлов при следующем сетевом запросе. Новые ответы этого
маршрута не сохраняются в кеше, поэтому состояние владельца проверяется при
каждом запросе. Ответ, полученный до установки этого изменения со старым
`max-age=86400`, может оставаться в кеше клиента до окончания прежнего срока.
Флаг `is_visible` управляет присутствием анкеты в поиске и не удаляет
фотографию у уже связанных с преподавателем учеников.
