...
> **Архивный документ.** PHP-код ниже не является действующим backend и не
> размещается на сервере. Актуальная реализация находится в
> `server/src/modules/auth`, а состояние переноса — в
