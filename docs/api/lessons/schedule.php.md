...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT
            ...
            classroom_sessions.status AS classroom_status,
            classroom_sessions.started_at AS classroom_started_at,
            classroom_sessions.ended_at AS classroom_ended_at,
        ...
        LEFT JOIN lesson_sessions AS classroom_sessions
            ON classroom_sessions.lesson_id = lessons.id
    ...
    foreach ($schedule as &$lesson) {
        ...
        $lesson['classroom_started_at'] = lessonFromStorageDateTime(
            $lesson['classroom_started_at'] ?? null,
            $viewerTimezone
        );
        $lesson['classroom_ended_at'] = lessonFromStorageDateTime(
            $lesson['classroom_ended_at'] ?? null,
            $viewerTimezone
        );
    ...
    successResponse([
        ...
        'period' => [
            ...
            'viewer_now' => (new DateTimeImmutable(
                'now',
                lessonPlatformTimezone()
            ))
