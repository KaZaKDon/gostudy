...
> **Архивный документ.** Описывает прежнюю реализацию на PHP/PHPMailer и
> используется только для переноса шаблонов и бизнес-правил. Актуальная почта
> реализуется в `server/src/modules/mail` на NestJS и Nodemailer.
...
6. После перехода по ссылке:
   ...
   - очищать срок действия, сохраняя токен для идемпотентного повторного
     открытия уже использованной ссылки.
