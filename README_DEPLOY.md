...
Все серверные файлы в пакете даны полностью. Соответствие путей:

- `docs/api/<path>/name.php.md` → `/api/<path>/name.php` на хосте;
- `docs/admin/<path>/name.php.md` → `/api/admin/<path>/name.php` на хосте.

При размещении убирается только конечный суффикс `.md`. Промежуточной папки
`backend` на хосте и в актуальном дереве снимков нет.
